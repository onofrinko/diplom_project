<section>
    ...
    <form method="post" action="{{ route('property.store') }}" class="mt-6 space-y-6">
        @csrf
        @method('post')
        ...
        <div>
            ...
            <x-text-input id="property_cost" name="property_cost" type="number" class="mt-1 block w-full" :value="old('property_cost')" required autocomplete="property_cost" />
            <x-input-error class="mt-2" :messages="$errors->get('property_cost')" />
        </div>

        <div>
            <x-input-label for="total_area" :value="__('Total Area (sq ft)')" />
            <x-text-input id="total_area" name="total_area" type="number" class="mt-1 block w-full" :value="old('total_area')" required autocomplete="total_area" />
        ...
        <div>
            ...
            <select id="property_status" name="property_status" class="mt-1 block w-full">
                <option value="pending" {{ old('property_status') == 'sold' ? 'selected' : '' }}>Pending</option>
        ...
        <div>
            ...
            <select id="property_type_id" name="property_type_id" class="mt-1 block w-full">
                ...
                <option value="{{ $type->property_type_id }}" {{ old('property_type_id') == $type->property_type_id ? 'selected' : '' }}>
                {{ $type->type }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mt-4">
            ...
            <x-text-input id="address_city" name="property_details[address][city]" type="text" class="mt-1 block w-full" :value="old('property_details.address.city')" required />
        ...
        <div>
            ...
            <x-text-input id="address_building" name="property_details[address][building]" type="text" class="mt-1 block w-full" :value="old('property_details.address.building')" required />
        ...
        <div>
            ...
            <x-text-input id="address_street" name="property_details[address][street]" type="text" class="mt-1 block w-full" :value="old('property_details.address.street')" required />
        ...
        <div>
            ...
            <x-text-input id="address_zip" name="property_details[address][zip]" type="text" class="mt-1 block w-full" :value="old('property_details.address.zip')" required />
            <x-input-error class="mt-2" :messages="$errors->get('property_details.address.zip')" />
        </div>

        <div>
            <x-input-label for="bedrooms" :value="__('Bedrooms')" />
            <x-text-input id="bedrooms" name="property_details[bedrooms]" type="number" class="mt-1 block w-full" :value="old('property_details.bedrooms')" required />
            <x-input-error class="mt-2" :messages="$errors->get('property_details.bedrooms')" />
        </div>

        <div>
            <x-input-label for="bathrooms" :value="__('Bathrooms')" />
            <x-text-input id="bathrooms" name="property_details[bathrooms]" type="number" class="mt-1 block w-full" :value="old('property_details.bathrooms')" required />
            <x-input-error class="mt-2" :messages="$errors->get('property_details.bathrooms')" />
        </div>

        <div>
            <x-input-label for="floors" :value="__('Floors')" />
            <x-text-input id="floors" name="property_details[floors]" type="number" class="mt-1 block w-full" :value="old('property_details.floors')" required />
        ...
        <div>
            ...
            <select id="garage" name="property_details[garage]" class="mt-1 block w-full">
                <option value="1" {{ old('property_details.garage') == true ? 'selected' : '' }}>Yes</option>
                <option value="0" {{ old('property_details.garage') == false ? 'selected' : '' }}>No</option>
            </select>
        ...
        <div>
            ...
            <select id="pool" name="property_details[pool]" class="mt-1 block w-full">
                <option value="1" {{ old('property_details.pool') == true ? 'selected' : '' }}>Yes</option>
                <option value="0" {{ old('property_details.pool') == false ? 'selected' : '' }}>No</option>
            </select>
            <x-input-error class="mt-2" messages=”$errors->get(‘property_details.pool’)” />

                <div>
                    <x-input-label for="description" :value="__('Description')" />
                    <textarea id="description" name="property_details[description]" class="mt-1 block w-full" required>{{ old('property_details.description') }}</textarea>
